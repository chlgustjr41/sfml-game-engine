...
void subscribeServerUpdate(EventManager *eventManager,
                           bool *isRunning,
        ...
        while (tokenIndex < tokenized.size())
        {
            ...
            else if (message.find(GameObject::CHARACTER_OBJECT_TYPE) != std::string::npos)
            {
                ...
                if (localPlayerIndex == -1)
                {
                    ...
                    CharacterObject *addingPlayer = new CharacterObject(eventManager, characterShape, characterBoundary, playerIndex);
...
int main(int argc, char *argv[])
{
    ...
    ViewObject *view = new ViewObject(eventManager, viewBoundary, localClient, localCurrentClientIndex);
    ...
    bool isRunning = true;
    std::thread subscribeServerUpdateThread = std::thread(subscribeServerUpdate, eventManager, &isRunning, &players, &movingPlatforms, &playersMutex, &movingPlatformsMutex);
    while (isRunning)
    {
        if (window->hasFocus())
        {
            ...
            // Create local character input event
            if (inputMsg.length() > 2)
            {
                eventManager->registerEvent(new EventObject(EventObject::CHARACTER_INPUT,
                                                            ...
                                                            players[localCurrentClientIndex]->getHeader(),
                                                            inputMsg));
            }

            // Polling input event and sending its content across the network
            EventObject *inputEvent = eventManager->pollEvent(EventObject::CHARACTER_INPUT, players[localCurrentClientIndex]->getHeader());
            if (inputEvent != NULL)
            {
                for (int i = 0; i < BURST_PUBLISH_COUNT; i++)
                    publisherSocket.send(zmq::buffer(inputEvent->getContent()), zmq::send_flags::none);
            }
