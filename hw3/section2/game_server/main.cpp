...
void publishGameState(std::vector<std::string> *userInputs,
...
int main()
{
    ...
    DeathZoneObject *deathZone = new DeathZoneObject(deathZoneShape, spawnZoneShape, &players);

    // Publish game state which includes moving platforms and user input response
    std::thread publishStateThread = std::thread(publishGameState, &userInputs, &platforms, &players, deathZone, &userInputsMutex, &playersMutex);
    ...
    while (isRunning)
    {
        ...
        if (tokenized[0] == CLIENT_CONNECTION_REQUEST_HEADER)
        {
            ...
            playersMutex.lock();
            CharacterObject *player = new CharacterObject(characterShape, characterBoundary, clientIndex, CHARACTER_COLORS[clientIndex % CHARACTER_COLORS.size()], &platforms);
