...
void CharacterThread::run()
{
    ...
    {
        (*this).setPosition((*this).getPosition().x - CHARACTER_MOVEMENT, (*this).getPosition().y);
    }
    ...
    if (isJumping)
    {
        ...
        (*this).move(0, -JUMP_HEIGHT * std::cos(M_PI * jumpTime / JUMP_FRAME));
        ...
        isOnMovingPlatform)
    {
        ...
        if ((*movingPlatform).getDirectionFlag())
            (*this).setPosition((*this).getPosition().x + (*movingPlatform).MOVING_PLATFORM_MOVEMENT, (*this).getPosition().y);
        else
            (*this).setPosition((*this).getPosition().x - (*movingPlatform).MOVING_PLATFORM_MOVEMENT, (*this).getPosition().y);
    }
}