...
const double ThreadObject::TIME_RATIO = 0.5;

ThreadObject::ThreadObject(std::string name,
                           const sf::Vector2f &wSize,
                           const sf::Vector2f &size,
                           const sf::Vector2f &position) : windowSize(wSize), m_size(size), threadName(name), duration(0)
{
    setSize(m_size);
...
double ThreadObject::getDuration()
{
    return duration;
}
...
void ThreadObject::setDuration(double newDuration)
{
